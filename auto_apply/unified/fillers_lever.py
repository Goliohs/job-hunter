...
class LeverFiller(ATSBaseFiller):
    ...
    async def fill(self) -> FillResult:
        ...
        # 6. Última pasada: required que sigan vacíos tras las custom questions
    ...
    async def _answer_custom_questions(self):
        ...
        questions = await self.page.query_selector_all(
            ".application-question, [data-qa*='question'], div.question"
        )
        for q in questions:
            try:
                ...
                # Radios: elegir opción cuyo texto matchee la respuesta
                radios = await q.query_selector_all('input[type="radio"]')
                if radios:
                    if answer is None:
                        # EEO u opt-out: marcar primera opción tipo "prefer not"/"decline" si existe
                        await self._pick_radio(radios, ["prefer not", "decline", "do not"])
                    else:
                        picked = await self._pick_radio(radios, [answer.lower()])
                        if not picked and answer.lower() in ("yes", "no"):
                            picked = await self._pick_radio_by_yesno(radios, answer)
                        if picked:
                            self.filled[label_text[:60]] = answer
                    continue
                ...
                logger.debug(f"Lever question error: {e}")
    ...
    async def _pick_radio(self, radios, keywords) -> bool:
        for r in radios:
            try:
                ...
                if not parent_text:
                    pp = await r.evaluate_handle("el => el.closest('label') || el.parentElement")
                    el = pp.as_element()
                    if el:
                        parent_text = ((await el.inner_text()) or "").lower()
                if any(kw in parent_text for kw in keywords):
                    await r.check()
                    return True
            except Exception:
                continue
